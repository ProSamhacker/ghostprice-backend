    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (
                ...
                ?,
                COALESCE(
                    (SELECT first_seen_at FROM tracked_products WHERE asin = ?),
                    CURRENT_TIMESTAMP
                ),
                CURRENT_TIMESTAMP
            )
        """, (asin, product_title, category, marketplace, currency, asin))
        ...
        return {
            ...
            "price_data": {
                ...
                "lowest_30d": round(stats[1], 2) if stats[0] > 0 and stats[1] else current_price,
                "highest_30d": round(stats[2], 2) if stats[0] > 0 and stats[2] else current_price,
                "avg_30d": round(stats[3], 2) if stats[0] > 0 and stats[3] else current_price,
                "data_points": stats[0]
            }
        }
